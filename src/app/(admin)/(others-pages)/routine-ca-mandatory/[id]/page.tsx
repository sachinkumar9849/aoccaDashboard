...
const UpdateClassPage = () => {
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      await queryClient.cancelQueries({ queryKey: ['ca-mandatory-training-list'] });

      // Snapshot the previous value
      const previousClasses = queryClient.getQueryData(['ca-mandatory-training-list']);
    ...
    onError: (error: Error, variables, context) => {
      ...
      if (context?.previousClasses) {
        queryClient.setQueryData(['ca-mandatory-training-list'], context.previousClasses);
      }
  ...
  return (
    <form key={classData?.data?.id || "loading"} onSubmit={formik.handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-5">
        <ComponentCard title="CA Mandatory routine update">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="col-span-2 flex mt-3 gap-2">
              ...
              <button
                type="button"
                onClick={() => router.push("/ca-mandatory-training-list")}
