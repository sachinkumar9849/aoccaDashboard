...
const CaFoundationList: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useQuery<NewsBlog[]>({
    ...
    queryFn: async () => {
      const response = await apiClient.request<ApiResponse>('/classes?type=CA-Intermediate&status=true');
  ...
  return (
    <div className="w-full p-4 bg-white rounded-lg shadow-sm">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-normal mb-4">CA Foundation Classes List</h2>
        <Link href="/ca-intermediate-routine">
          <Button>Add</Button>
        </Link>
      </div>
