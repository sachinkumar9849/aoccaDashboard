...
const GalleryList: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useQuery<NewsBlog[]>({
    ...
    queryFn: () => apiClient.request<NewsBlog[]>('/toper-testimonial-team?type=gallery')
  });
  ...
  return (
    <div className="w-full p-4 bg-white rounded-lg shadow-sm">
      <h2 className="text-lg font-normal mb-4">Gallery List</h2>
                ...
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-6">
                    <div className="flex gap-2">
                      <Link href={`/gallery-list/${item.id}`} className="p-1 text-blue-500 hover:text-blue-700" title="Edit">
                        <Edit size={18} />
                      </Link>
                      <button
