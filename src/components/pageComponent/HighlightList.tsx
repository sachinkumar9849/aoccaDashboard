...
const HighlightList: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useQuery<NewsBlog[]>({
    queryKey: ['highlight-list'],
    queryFn: () => apiClient.request<NewsBlog[]>('/toper-testimonial-team?type=highlight')
  });
  ...
  return (
    <div className="w-full p-4 bg-white rounded-lg shadow-sm">
      <h2 className="text-lg font-normal mb-4">Highlights List</h2>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b">
              <th className="py-3 px-6 text-left text-sm font-medium text-gray-600">Image</th>
              <th className="py-3 px-6 text-left text-sm font-medium text-gray-600">Video Link</th>
                ...
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-6">
                    <div className="flex gap-2">
                      <Link href={`/highlight-list/${item.id}`} className="p-1 text-blue-500 hover:text-blue-700" title="Edit">
                        <Edit size={18} />
                      </Link>
                      <button
