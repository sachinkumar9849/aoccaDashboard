...
const fetchSliders = async (): Promise<SliderItem[]> => {
    try {
        return await apiClient.request<SliderItem[]>('/toper-testimonial-team?type=team&status=published');
    } catch (error) {
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b">
                            <th className="py-2 px-6 text-left text-sm font-medium text-gray-600">Sn</th>
                            <th className="py-2 px-6 text-left text-sm font-medium text-gray-600">Title</th>
                            <th className="py-2 px-6 text-left text-sm font-medium text-gray-600">Name</th>
                            <th className="py-2 px-6 text-left text-sm font-medium text-gray-600">Image</th>

                            <th className="py-2 px-6 text-left text-sm font-medium text-gray-600">Status</th>
                            <th className="py-2 px-6 text-left text-sm font-medium text-gray-600">Created At</th>
                    ...
                    <tbody>
                        {data.map((item, index) => (
                            <tr key={item.id} className="border-b hover:bg-gray-50">
                                <td className="py-2 px-6 text-gray-700  w-[50px]">{index + 1}</td>
