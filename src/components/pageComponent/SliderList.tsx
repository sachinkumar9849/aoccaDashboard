...
const SliderList: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useQuery<NewsBlog[]>({
    queryKey: ['slider-list'],
    queryFn: () => apiClient.request<NewsBlog[]>('/toper-testimonial-team?type=slider')
  });
  ...
  return (
    <div className="w-full p-4 bg-white rounded-lg shadow-sm">
      <h2 className="text-lg font-normal mb-4">Slider List</h2>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b">
            ...
            {filteredItems && filteredItems.length > 0 ? (
              filteredItems.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-6">
                    <div className="flex gap-2">
                      <Link href={`/slider-list/${item.id}`} className="p-1 text-blue-500 hover:text-blue-700" title="Edit">
                        <Edit size={18} />
                      </Link>
                      <button
